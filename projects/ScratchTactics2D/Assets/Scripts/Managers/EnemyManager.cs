...
{
	public List<OverworldEnemyBase> enemyList;
	...
	void Awake() {
		...
		enemyList = new List<OverworldEnemyBase>();
		...
		for (int i = 0; i < enemyList.Count; i++) {		
			switch(enemyList[i].state) {
				...
				case Enum.EnemyState.followField:
					enemyList[i].FollowField(flowFieldToPlayer, GameManager.inst.player);
					break;
			}
	...
	public void SpawnNew() {
		// select which prefab variant to instantiate
		var enemy = OverworldEnemySkeleton.Spawn(overworldEnemySkeletonPrefab);
		AddSubject(enemy);
	}
	...
	public void AddSubject(OverworldEnemyBase enemy) {
		enemyList.Add(enemy);
	}
